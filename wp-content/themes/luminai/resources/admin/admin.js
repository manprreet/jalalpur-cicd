...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".luminai-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.luminai_admin) {
    return;
  }

  var _window$luminai_admin = window.luminai_admin,
      builderStatusData = _window$luminai_admin.builderStatusData,
      getStartedData = _window$luminai_admin.getStartedData;
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".luminai-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.luminai-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".luminai-admin-big-notice").addClass("processing");
      $(".luminai-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".luminai-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.luminai-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.luminai-admin-big-notice').on("click", ".luminai-notice-dont-show-container", disableNotice);
  ...
  window.luminai_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
