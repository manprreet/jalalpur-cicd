...
function luminai_register_components($components) {
...
Hooks::prefixed_add_filter('components', 'luminai_register_components', 20);
Theme::load(
	array(
 ...
 * @return Theme
 */
function luminai_theme() {
	return Theme::getInstance();
}

function luminai_assets() {
	return luminai_theme()->getAssetsManager();
}
...
luminai_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__('Header Menu', 'luminai'),
			'footer-menu' => esc_html__('Footer Menu', 'luminai'),
		)
	);
...
if (!apply_filters('kubio_is_enabled', false)) {
	luminai_assets()
		...
		->registerStylesheet('luminai-theme', '/theme/theme.css');
	luminai_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		luminai_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action('wp_enqueue_scripts', 'luminai_print_color_scheme', 0);
} else {
	luminai_assets()
		->registerStyle('luminai-theme', luminai_assets()->getBaseURL() . '/theme/fse-base-style.css');
	Hooks::prefixed_add_filter('skip_google_fonts', '__return_true');
}

function luminai_add_woocommerce_suppoert() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action('after_setup_theme', 'luminai_add_woocommerce_suppoert');
...
function luminai_get_builder_plugin_slug() {
...
function luminai_theme_plugins($plugins) {

	$plugin_slug = luminai_get_builder_plugin_slug();

	$plugins[$plugin_slug] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __('Kubio PRO', 'luminai') : __('Kubio', 'luminai'),
...
Hooks::prefixed_add_filter('theme_plugins', 'luminai_theme_plugins');

Hooks::prefixed_add_action(
	'after_plugin_activated',
	function ($slug) {

		if ($slug === luminai_get_builder_plugin_slug()) {
			...
			<div class="kubio-admin-row get-started-imported notice notice-success">
				<div>
					<p class="imported-title">
						...
						echo esc_html(
							sprintf(
								__('%s design has been successfully imported!', 'luminai'),
								luminai_theme()->getName()
							)
						);
						?>
					</p>
					<p class="imported-subtitle">
						...
						echo esc_html(
							sprintf(
								__('%s design has been successfully imported! You can take a look at your new design or start editing it', 'luminai'),
								luminai_theme()->getName()
							)
						);
						?>
				...
				<div class="button imported-view-site-button">
					<a href="<?php echo esc_url(site_url()); ?>">
						<?php echo esc_html(__('View site', 'luminai')); ?>
					</a>
...
function luminai_try_online_url() {
	return "https://kubiobuilder.com/go/try-theme/" . get_template();
}

add_filter('kubio/editor-try-online/url', 'luminai_try_online_url', 10);

function luminai_render_header_style() {

	luminai_theme()->get('css')->render();
}
add_action('wp_head', 'luminai_render_header_style', 100);
